...
from model.utils.FE import FE
from model.utils.FF import FF
from model.utils.FE_rgb import FE_rgb
from model.utils.FF_rgb import FF_rgb
...
class Encoder(nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(cfg.num_stages):
            ...
            setattr(self, f"fe{i + 1}", FE(cfg.FE, i))
            setattr(self, f"fe_rgb{i + 1}", FE_rgb(cfg.FE, i))
            setattr(self, f"ff{i + 1}", FF(cfg.FF, i))
            setattr(self, f"ff_rgb{i + 1}", FF_rgb(cfg.FF, i))
        self.apply(self._init_weights)
    ...
    def forward_features(self, rgb, depth):
        ...
        for i in range(self.num_stages):
            if i == 0:
                ...
                rgb, depth = getattr(self, f"fe{i + 1}")(rgb, depth)
                out = getattr(self, f"ff{i + 1}")(rgb, depth)
                ...
                rgb, depth = getattr(self, f"fe_rgb{i + 1}")(rgb, depth)
                out = getattr(self, f"ff_rgb{i + 1}")(rgb, depth)
...
class RGBD_seg(nn.Module):
    def __init__(self, cfg):
        super(RGBD_seg, self).__init__()
...
if __name__ == '__main__':
    Config = get_config()
    model = RGBD_seg(cfg=Config).to("cuda")
    model.eval()
