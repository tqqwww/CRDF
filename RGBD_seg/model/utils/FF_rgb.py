...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, N, C = rgb.shape
        rgb_q = self.rgb_q(rgb).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        if self.sr_ratio > 1:
            ...
            rgb_kv = self.rgb_kv(x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        rgb_v = (rgb_attn @ rgb_v).transpose(1, 2).reshape(B, N, C)

        x = torch.cat([rgb_v, depth], dim=1)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class FF_RGB(nn.Module):
    def __init__(self, cfg, stage):
        super(FF_RGB, self).__init__()
    ...
    def forward(self, rgb, depth):
        ...
        out = self.bn_norm(self.proj(x) + self.residual(x))
